# http://stackoverflow.com/a/14615347/1234408
...
def find_circles(frame):
    ...
    circles, obj_center = [], []
    contours = cv2.findContours(components,
            cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

    for c in contours:
        c = numpy.asarray(c, numpy.int64) # XXX OpenCV bug.
        print(c)
...
def track_center(objcenter, newdata):
    for i in xrange(len(objcenter)):
        ...
        if i == j:
            ...
            print("Swapping {0} <-> {1}".format((i, objcenter[i]), (j, objcenter[j])))
